...
class AsyBrainControlCenter(Thread):
    ...
    def SpeedVectorUpdata(self, index, direction):
        '''
        更新无人机的速度向量
        :param index:
        :param direction (1 or -1)代表方向
        :return:
        '''
        for i in range(4):
            if i == index:
                if self.SpeedVectors[index] == 4*direction:
                    self.SpeedVectors[index] = 4*direction
                else:
                    self.SpeedVectors[index] += 1*direction

            else:
                if self.SpeedVectors[i] == 0:
                    self.SpeedVectors[i] = 0
                else:
                    self.SpeedVectors[i] = np.sign(self.SpeedVectors[i])*(np.abs(self.SpeedVectors[i])-1)
    ...
    def SpeedUpdata(self, command):
        ...
        # 左右移动
            ...
            if np.abs(self.SpeedVectors[0]) == 4 or np.abs(self.SpeedVectors[1]) == 4 or np.abs(self.SpeedVectors[2]) == 4 or np.abs(self.SpeedVectors[3]) == 4:
                self.set_speed(self.DronSpeed)
            else:
